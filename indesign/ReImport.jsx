...
function ri_prep_layers(ri) {
  if (!ri.ui_epubbify.checkedState)
    return;

  ri.layers = {};
  ri_remake_layer(ri, "rasterized");
  ri_remake_layer(ri, "non_epub");
  ri.layers.body = ri.doc.layers.lastItem();
}

function ri_remake_layer(ri, name) {
  ri.layers[name] = ri_do_remake_layer(ri, name)
}

function ri_do_remake_layer(ri, name) {
  var full_name = "ri:(" + name + ")"
  var layer = ri.doc.layers.itemByName(full_name)
  if (layer && layer.isValid) {
    try {
      layer.remove();
    } catch (err) {
      ri_logw("Cannot remove layer \"" + full_name + "\": " + err)
      return layer;
    }
  }
...
function ri_do_import(ri) {
  ...
  try {
    ...
    ri_logw(ri, 'Could not import document: ' + e);
  }
...
function riss_do_mirrors(ri) {
  ...
  var epubbify = ri.ui_epubbify.checkedState;
  var deleteOriginal = !epubbify;

  for (var i = 0; i < hits.length; ++ i) {
    var hit = hits[i];
    try {
      var outlines = hit.createOutlines(deleteOriginal);
      for (var j = outlines.length - 1; j >= 0; -- j) {
        var outline = outlines[j];
        outline.flipItem(Flip.HORIZONTAL);
        if (epubbify)
          outline.itemLayer = ri.layers.non_epub;
      }
      if (epubbify)
        hit.fillColor = "None";
      count ++;
  ...
  if (count > 0) {
    ...
    if (epubbify)
      ri_logw(ri, "Remember to hide layer \"" + ri.layers.non_epub.name + "\" when exporting to epub.");
  }
}
...
function ri_groom_rasterize(ri) {
  ...
  for (i = src_frames.length - 1; i >= 0; -- i) {
    ...
    dst_frame = src_frame.duplicate(ri.layers.rasterized);
