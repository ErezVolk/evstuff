      ...
      with (ri.uig_post = enablingGroups.add({staticLabel: "Post-Import", checkedState: true})) {
        with (dialogColumns.add() ) {
          ...
          ri.ui_post_convert_post_its =
            checkboxControls.add({
              staticLabel: "Convert Tagged Text comments to Post-It notes",
              checkedState: true
            });
        }
      }
...
function ri_post_convert_post_its(ri) {
  ...
  try {
    var group = ri.doc.characterStyleGroups.item("(autogenerated)");
    var crstyle = group.characterStyles.item("(Comment Reference)");
  } catch (e) {
    return;
  }
  ...
  app.findGrepPreferences.findWhat = "~F";
  app.findGrepPreferences.appliedCharacterStyle = crstyle;

  //alert(app.findGrep().text);
}
