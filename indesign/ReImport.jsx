...
function ri_create_new_epub_stuff(ri) {
  if (!ri.ui_epubbify.checkedState)
    return;

  ri_create_layer(ri, "non_epub");
  ri_create_layer(ri, "rasterized").move(LocationOptions.AT_END);
}

function ri_create_layer(ri, name) {
  var full_name = "ri:(" + name + ")";
  var layer = ri.doc.layers.add({name: full_name});
  ri.for_epub["layer_" + name] = layer;
  return layer;
}
  ...
  for (var i = 0; i < frames.length; ++ i) {
    try {
      ...
      if (!layer || !layer.isValid)
        continue;
      if (FRAME_NAME_RE.test(layer.name))
        continue;
...
function riss_do_mirrors(ri) {
  ...
  var outlines, epubbify = ri.ui_epubbify.checkedState;
  ...
  for (var i = 0; i < hits.length; ++ i) {
    ...
    try {
      ...
      for (var j = 0; j < outlines.length; ++ j) {
        ...
        if (epubbify) {
          outline.itemLayer = ri.for_epub.layer_non_epub;
  ...
  if (count > 0) {
    ...
    if (epubbify)
      ri_logw(ri,
        "When exporting to epub, either hide the layer \"" + ri.for_epub.layer_non_epub.name + "\" " +
...
function ri_groom_rasterize(ri) {
  ...
  var dst_layer = ri.for_epub.layer_rasterized;
...
function ri_get_all_frames_with_para_styles_containins(ri, substr) {
  ...
  for (var i = 0; i < paras.length; ++ i) {
    ...
    if (!frame.itemLayer || !frame.itemLayer.isValid)
      continue;
    if (FRAME_NAME_RE.test(frame.itemLayer.name))
      continue;
    var fid = frame.id;
