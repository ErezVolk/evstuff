...
function ri_post_it_object_style(ri) {
  ...
  var style = styles.item(BALLOON_STYLE_NAME);
  if (style && style.isValid)
    return style;

  style = styles.add({
    name: BALLOON_STYLE_NAME,
    nonprinting: true,
    ...
    enableTextFrameColumnRuleOptions: true,
...
function ri_post_convert_hyperlinks(ri) {
  ...
  var style = ri.doc
    .characterStyleGroups.item(AUTOGENERATED_GROUP_NAME)
    .characterStyles.item("(Hyperlink)");
  if (style && style.isValid)
    count += ri_post_convert_hyperlinks_style(ri, style);
  ...
  if (count > 0)
    ri_log(ri, "Converted " + count + " hyperlink(s) to QR(s)");
}

function ri_post_convert_hyperlinks_style(ri, style) {
  var count = 0;
  ...
  for (var i = hits.length - 1; i >= 0; -- i)
    if (ri_convert_hyperlink(ri, hits[i]))
      count += 1
  return count;
}

function ri_convert_hyperlink(ri, text) {
  var doc = ri.doc;
  var url = text.contents;

  // Make the rectangle
  var len = doc.gridPreferences.baselineDivision * 3;
  var frame = text.parentTextFrames[0];
  if (frame && frame.isValid) {
    var style = frame.appliedObjectStyle;
    if (style && style.isValid && style.name == BALLOON_STYLE_NAME) {
      var bounds = frame.geometricBounds;
      frame.textFramePreferences.insetSpacing = 0;
      len = (bounds[3] - bounds[1]);
    }
  }
  rect = doc.rectangles.add({
    geometricBounds: [0, 0, len, len],
    strokeColor: "None",
    // strokeColor: doc.colors.itemByName("Magenta")
  });

  // Fill it with a QR code
  // rect.createHyperlinkQRCode(url, "Magenta");
  rect.createHyperlinkQRCode(url);

  // Make it clickable
  hlpiss = doc.hyperlinkPageItemSources;
  hlpis = hlpiss.add(rect);

  hluds = doc.hyperlinkURLDestinations;
  hlud = hluds.add(url);
  doc.hyperlinks.add(hlpis, hlud);

  // Replace the text with it:
