...
class DocxShuffle(DocxWorker):
    ...
    def make_rerunner(self) -> None:
        ...
        args = self.args
        if not args.make_rerunner:
            return
        rerunner = args.input.with_name(args.input.name + ".rerun")
        print(f"Writing {rerunner}")
        with rerunner.open("wt", encoding="utf-8") as fobj:
            fobj.write("#!/bin/bash\n# AUTOGENERATED FILE, DO NOT EDIT.\n\n")
            cli = [self._quote(__file__), self._quote(args.input), "--make-rerunner"]
            self._add_path(cli, "model")
            self._add_flag(cli, "one_time_seed")
            self._add_flag(cli, "keep_comments")
            self._add_flag(cli, "destyle")
            self._add_arg(cli, "contains")
            self._add_arg(cli, "max_length")
            self._add_arg(cli, "restyle")
            fobj.write(" ".join(map(str, cli)))
        rerunner.chmod(0o755)

    def _add_flag(self, cli: list[str], name: str) -> None:
        """Add a flag if it was set."""
        if getattr(self.args, name):
            cli.append(self._argify(name))

    def _add_arg(self, cli: list[str], name: str) -> None:
        """Add a plain arg if it was set."""
        if (value := getattr(self.args, name)):
            cli.append(self._argify(name))
            cli.append(shlex.quote(str(value)))

    def _add_path(self, cli: list[str], name: str) -> None:
        """Add a path if it was set."""
        if (value := getattr(self.args, name)):
            cli.append(self._argify(name))
            cli.append(self._quote(value))
